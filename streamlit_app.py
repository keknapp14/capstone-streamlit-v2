...
def load_data():
    ...
    best = pd.read_csv("best_model_per_product.csv")
    arr = pd.read_csv("arr_by_product.csv")
    renewal = pd.read_csv("renewal_by_product.csv")
    cltr = pd.read_csv("cltr_by_income.csv")
...
all_products = sorted(predictions["PRODUCT"].unique())
selected_products = st.sidebar.multiselect(
    "Products", options=all_products, default=all_products
)
...
with tab1:
    ...
    predict_product = st.selectbox(
        "Select Product", options=sorted(filtered["PRODUCT"].unique()),
        key="pred_product"
    )
    ...
    c2.metric("Avg Churn Prob", f"{product_preds['CHURN_PROBABILITY'].mean():.1%}")
    c3.metric("Actual Churn Rate", f"{(1 - product_preds['RENEWED'].mean()):.1%}")
    c4.metric("High Risk (Q4)", f"{(product_preds['RISK_QUARTILE'] == 'Q4 (High Risk)').sum():,}")
    ...
    fig_q = px.bar(
        q_summary, x="RISK_QUARTILE", y="ACTUAL_CHURN_RATE",
        text_auto=".1%", title=f"{predict_product} — Actual Churn by Risk Quartile"
    )
    ...
    st.dataframe(
        product_preds.nlargest(100, "CHURN_PROBABILITY")[[
            "CUSTOMER_ID", "CHURN_PROBABILITY", "RENEWAL_PROBABILITY",
            "RISK_QUARTILE", "RENEWED"
        ]],
...
with tab2:
    ...
    fig_comp = px.bar(
        ...
        x="PRODUCT", y="ROC_AUC", color="MODEL",
        barmode="group", title="ROC AUC by Model and Product",
        text_auto=".3f"
    )
    ...
    st.dataframe(best_df[best_df["PRODUCT"].isin(selected_products)], use_container_width=True)

# --- Tab 3: Risk Distribution ---
with tab3:
    st.header("Churn Probability Distribution")
    fig_hist = px.histogram(
        filtered, x="CHURN_PROBABILITY", color="PRODUCT",
        nbins=50, title="Distribution of Predicted Churn Probabilities",
        opacity=0.7
    )
    ...
    cross_q = (
        filtered.groupby(["PRODUCT", "RISK_QUARTILE"], observed=True)
        .agg(ACTUAL_CHURN_RATE=("RENEWED", lambda x: 1 - x.mean()))
        .reset_index()
    )
    fig_cross = px.bar(
        cross_q, x="RISK_QUARTILE", y="ACTUAL_CHURN_RATE",
        color="PRODUCT", barmode="group", text_auto=".1%"
    )
...
with tab4:
    ...
    with col1:
        arr_df["YEAR_DATE"] = pd.to_datetime(arr_df["YEAR_DATE"])
        fig_arr = px.line(
            arr_df[arr_df["PRODUCT"].isin(selected_products)],
            x="YEAR_DATE", y="ARR", color="PRODUCT",
            title="Annual Recurring Revenue by Product", markers=True
        )
        fig_arr.update_yaxes(tickprefix="$", tickformat=",.0f")
        st.plotly_chart(fig_arr, use_container_width=True)
    with col2:
        renewal_df["YEAR_DATE"] = pd.to_datetime(renewal_df["YEAR_DATE"])
        fig_ren = px.line(
            renewal_df[renewal_df["PRODUCT"].isin(selected_products)],
            x="YEAR_DATE", y="RENEWAL_RATE", color="PRODUCT",
            title="Customer Renewal Rate by Product", markers=True
        )
    ...
    fig_cltr = px.bar(
        cltr_df, x="INCOME_LEVEL", y="AVERAGE_CLTR",
        title="Average Customer Lifetime Revenue by Income Level",
        text_auto="$,.0f"
    )
    st.plotly_chart(fig_cltr, use_container_width=True)

# --- Tab 5: Customer Lookup ---
with tab5:
    st.header("Customer Lookup")
    search_id = st.number_input("Enter Customer ID", min_value=0, step=1)
    if search_id > 0:
        customer_rows = predictions[predictions["CUSTOMER_ID"] == search_id]
